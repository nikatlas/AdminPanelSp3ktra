...
var vat = 1.19 - 0.19 * document.getElementById('vat').checked;
var biig = 45 * document.getElementById('big').checked;
cost =  ( cp  + our_shipping + pc + insurance + biig ) * vat;
cost = cost.toFixed(2);
...
<?php
	$model = new \models\ebay\package();
	$ss = 0;
	foreach( (array)$data['package'] as $item){
		$ss += \helpers\currency::convert($item->currentprice + $item->shippingcost,$item->currency , 'EUR');
?>            
            <tr style="height:100px;background:#EEF1FF;">
                <td width="18%">
                    <a  target="_new" href="<?php echo $item->url;?>" data-lightbox="<?php echo $item->itemid;?>" title=""><img src="<?php echo $item->imgurl;?>"></a>
                    <br />
                    <a href="/ebay/package/removeFromPackage/<?php echo $item->itemid;?>/<?php echo $data['id'];?>">UnPackage</a>    
                </td>
					...
					<tbody>
        	    		<tr>
							...
							<td valign="middle" style="text-align:right">
								<span ><? echo $item->currentprice." ".strtoupper($item->currency);?></span>
							</td>
							<td></td>
						</tr>
          				<tr>
                            <td width="60%" valign="middle"><b style="font-size:15px;float:right;">Price inc. shipping:</b></td>
                            <td width="30%" valign="middle" style="text-align:right"><span class="large bold"><? echo \helpers\currency::convert($item->currentprice + $item->shippingcost,$item->currency , 'EUR'); ?></span>
           					</td>
							<td>€</td>
						</tr>
                        <tr class="plus">
...
<!------------------------------------------------------------------------------------------->
<?php $pack = $data['pack'];?>
<input type="hidden" name="price" id="price" value="<?php echo $ss;?>"/>

			<tr style="height:50px;border:1px solid #CECECE;"> 
			
               <form action="/ebay/package/save/<?php echo $data['id'];?>" method="post">
				...
				<td width="18%" style="text-align:center">
          			    <div style="float:left;"> <span> - 19 %</span><br />
							<input type="checkbox" onchange="changeSelect();" name="vat" id="vat" <?php if( $pack->vat == 1 ) echo 'checked';?>/>
            			</div>
                        <div style="float:left;"> <span> Big Dim.</span><br />
							<input type="checkbox" onchange="changeSelect();" name="big" id="big" <?php if( $pack->big == 1 ) echo 'checked';?>/>
            			</div>
						<span style="color:#969C9C">Weight:</span>  <br>
						<div class="styled">
							<select id="fweight" name="fweight" style="visibility: visible;" onchange="changeSelect();">
                                <option name="0.00" value="0" <?php if($pack->weight == 0 )echo 'selected=""';?>>0</option>
                                <option name="5.00" value="5" <?php if($pack->weight == 5 )echo 'selected=""';?>>0 - 0.5 kg</option>
                                <option name="10.00" value="10" <?php if($pack->weight == 10 )echo 'selected=""';?>>0.5 - 1kg</option>
                                <option name="20.00" value="20" <?php if($pack->weight == 20 )echo 'selected=""';?>>1-2kg</option>
                                <option name="40.00" value="40" <?php if($pack->weight == 40 )echo 'selected=""';?>>2-5kg</option>
                                <option name="60.00" value="60" <?php if($pack->weight == 60 )echo 'selected=""';?>>5-10kg</option>
                                <option name="90.00" value="90" <?php if($pack->weight == 90 )echo 'selected=""';?>>10-20kg</option>
                                <option name="120.00" value="120" <?php if($pack->weight == 120 )echo 'selected=""';?>>20-30kg</option>
                           </select>
						</div>
				</td>
				<td width="18%" style="text-align:center">
					<span style="color:#969C9C">Profit:</span>  <br>
					<input type="text" name="fprofit" value="<?php echo intval($pack->profit);?>" onfocus="this.value=''" onblur="verify_value()"  onkeypress="validate(event)" id="profit">	
				</td>
        	    ...
        	    <div class="styled">
					<select id="finsurance" name="finsurance" style="visibility: visible;" onchange="changeSelect();">
						<option value="0" <?php if($pack->insurancecost == 0 )echo 'selected=""';?>>0 €</option>
						<option value="1.5" <?php if($pack->insurancecost == 1.5 )echo 'selected=""';?>>100 €-&gt; 1.5 €</option>
						<option value="3" <?php if($pack->insurancecost == 3 )echo 'selected=""';?>>200 € -&gt; 3 €</option>
						<option value="4.5" <?php if($pack->insurancecost == 4.5 )echo 'selected=""';?>>300 € -&gt; 4.5 €</option>
						<option value="6" <?php if($pack->insurancecost == 6 )echo 'selected=""';?>>400 € -&gt; 6 €</option>
						<option value="7.5" <?php if($pack->insurancecost == 7.5 )echo 'selected=""';?>>500 € -&gt; 7.5 €</option>
						<option value="9" <?php if($pack->insurancecost == 9 )echo 'selected=""';?>>600 € -&gt; 9 €</option>
						<option value="10.5" <?php if($pack->insurancecost == 10.5 )echo 'selected=""';?>>700 € -&gt; 10.5 €</option>
						<option value="12" <?php if($pack->insurancecost == 12 )echo 'selected=""';?>>800 € -&gt; 12 €</option>
						<option value="13.5" <?php if($pack->insurancecost == 13.5 )echo 'selected=""';?>>900 € -&gt; 13.5 €</option>
						<option value="15" <?php if($pack->insurancecost == 15 )echo 'selected=""';?>>1000 € -&gt; 15 €</option>
						<option value="16.5" <?php if($pack->insurancecost == 16.5 )echo 'selected=""';?>>1100 € -&gt; 16.5 €</option>
						<option value="18" <?php if($pack->insurancecost == 18 )echo 'selected=""';?>>1200 € -&gt; 18 €</option>
						<option value="19.5" <?php if($pack->insurancecost == 19.5 )echo 'selected=""';?>>1300 € -&gt; 19.5 €</option>
						<option value="21" <?php if($pack->insurancecost == 21 )echo 'selected=""';?>>1400 € -&gt; 21 €</option>
						<option value="22.5" <?php if($pack->insurancecost == 22.5 )echo 'selected=""';?>>1500 € -&gt; 22.5 €</option>
					</select>
				</div>
                </td>
   				<td width="18%" style="text-align:center">	
...
<?php
	...
	foreach( (array)$data['local'] as $item){
...
?>            
            <tr style="height:100px;<?php echo (($item->pinned == 1) ? 'background:#EEF1FF;' : '');?> ">
                <td width="18%">
                    ...
                    <br />
                    <a href="/ebay/package/addToPackage/<?php echo $item->itemid;?>/<?php echo $data['id'];?>">Package</a>    
                    <? }?>
                </td>
                <td colspan="3" width="54%"><a id="title" class="bold"><?php echo $item->name;?></a><br>
                    <span class="xsmall" id="code">(<?php echo $item->itemid;?>)</span><br>
...
<? if ( strtoupper($item->currency) != 'GBP' && $item->pinned == 0) echo "<br><span class='xsmall'>GBP: ".\helpers\currency::convert($item->currentprice,$item->currency , 'GBP')."</span>";?>
						...
						<tr>
          				  <td></td>
          				  <td><a href="/ebay/package/removeItem/<?php echo $item->itemid;?>/<?php echo $data['id'];?>" style="float:right" class="btn btn-danger"  onclick="return confirm('Are you sure you want to delete this item? ');"  >Delete</a></td>
         				  <td></td>
          			   </tr>
						<?php }?>
					</tbody>
...
<br />
<br />
<a href="/ebay/package/delete/<?php echo $data['id'];?>" onclick="return confirm('Are you sure you want to delete all these? ');" style="float:right" class="btn btn-danger" >Delete All</a>

<form action="/ebay/package/addItems/<?php echo $data['id'];?>" method="post">
...
<a href="/notes/add/<?php echo $data['id'];?>"  style="float:left" class="btn btn-primary" >Add note</a>
<a href="/ebay/package/revise/<?php echo $data['id'];?>"  style="float:left" class="btn btn-primary" >Revise Prices</a>
